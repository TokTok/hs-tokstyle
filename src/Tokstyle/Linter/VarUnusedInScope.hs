...
-- scope.
combineAnd (Var (Operation NoReduce a) _ _) v2@Var{} | a^.flgNested > 0 = v2
...
--  trace (groom ((Var op1 decl1 use1), "AND", (Var op2 decl2 use2), "EQUALS", result) <> "\n") $
  ...
  where
    ...
    combineOp (Operation Declare a) (Operation act2 b)
      ...
      | otherwise =
          error (groom (Operation Declare a, Operation act2 b))
    ...
    combineOp _ (Operation NoReduce b) | b^.flgNested > 0 = op1
...
combineOr (Var op1 decl1 use1) (Var op2 decl2 use2) =
--  trace (groom ((Var op1 decl1 use1), "OR", (Var op2 decl2 use2), "EQUALS", result) <> "\n") $
  ...
  where
    ...
    combineOp (Operation act1 a) (Operation act2 b) | act1 == act2 = Operation act1 (a `orFlags` b){_flgConditional = False}
    -- Either written or read => consider it as conditional write.
    combineOp (Operation Write a) (Operation Read b) = Operation Write (a `orFlags` b)
    combineOp (Operation Read a) (Operation Write b) = Operation Write (a `orFlags` b)
    combineOp (Operation Write a) (Operation ReadWrite b) = Operation ReadWrite (a `orFlags` b)
    combineOp (Operation ReadWrite a) (Operation Write b) = Operation ReadWrite (a `orFlags` b)
    combineOp (Operation Read a) (Operation ReadWrite b) = Operation ReadWrite (a `orFlags` b)
    combineOp (Operation ReadWrite a) (Operation Read b) = Operation ReadWrite (a `orFlags` b)

    combineOp _ _ = error (groom (op1, op2))
