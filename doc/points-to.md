...
**Requirement:** The analysis must be *sound*. This means it must compute a
conservative over-approximation of the points-to sets. For any given pointer,
the analysis must identify *all* memory locations it could possibly point to
during any execution of the program. It is acceptable for the analysis to report
that a pointer *might* point to a location when it actually doesn't (a false
...
**Requirement:** The analysis will be *fully context-sensitive*. This means that
...
3.  **Benign Unsummarized External Functions:** For any external library
    ...
    critical assumption about its behavior:

    *   The function is assumed to have **no side effects on pointer values
        passed to it**. It may read from or write to the *data* at the memory
        location a pointer refers to, but it will not change the pointer itself
        to make it point to a different location.
    *   For example, a call to an unsummarized function `void process_data(int*
        ...
        `buffer` pointer *must* be manually summarized, as it violates this
        assumption.
...
5.  **Strict Field Access for Pointers in Unions:** When dealing with `union`
    ...
    *exact same field*. The code is assumed not to perform type punning on
...
*   **Store (`*p = q`):** If `PointsTo(p)` contains `UnknownLoc`, it means the
    store could modify *any* memory location. To model this, the analysis will
...
1.  **Data Structures:**
    ...
    *   **`FunctionSummary`**: Captures the effect of a function *in a specific
        context*. For this analysis, it primarily stores the points-to sets of
...
2.  **Algorithm:** a. **Initialization:** i. Build an initial `CallGraph` with
    ...
    **Check for Changes:** If this new summary is different from the one
    ...
    all *callers* of `f` (found via the `CallGraph`) back to the worklist, each
...
*   **External Function Calls:** Calls to functions whose source is not
    ...
    2.  **Conservative Fallback:** For any external function *not* in our
